...
object TaperPlan {
    ...
    /**
     * The average of complete recorded days. One canonical number — onboarding, the
     * Settings readout and the auto-seed all call this, so they can't disagree.
     * Returns 0 until there are at least [minDays] complete days.
     */
    fun historyAverage(c: Context, minDays: Int = 3): Int {
        val days = History.completeDays(c)
        if (days.size < minDays) return 0
        return days.average().roundToInt()
    }

    fun historyDayCount(c: Context): Int = History.completeDays(c).size
