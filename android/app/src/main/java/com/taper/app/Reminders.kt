...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        if (UsageRepo.hasPermission(context)) {
            History.record(context, LocalDate.now().toEpochDay(), UsageRepo.totalMinutes(context))
        }
        TaperPlan.finishMeasurementIfDue(context)
        if (!context.remindersOn) return
        if (TaperPlan.measuring(context)) return

        val target = TaperPlan.targetMinutes(context)
        if (target == TaperPlan.NO_TARGET) return
